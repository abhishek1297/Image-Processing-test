...
int hist_size[n_channels] {60, 32};
const float h_ranges[2] {0, 256};
const float s_ranges[2] {0, 180};
const float v_ranges[2] {0, 256};
const float* ranges[n_channels] {h_ranges, s_ranges};//, v_ranges};
const int channels[n_channels] {0, 1};


cv::Mat hist::extract(cv::Mat &&src) {

	cv::Mat hist_base;
	cv::Mat mask;
	try {
		cv::calcHist( &src, 1, channels, mask, hist_base, n_channels, hist_size, ranges, true, false);
